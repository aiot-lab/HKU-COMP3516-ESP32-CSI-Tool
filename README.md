...
* CSI extraction
Related files
- _components/csi_cal.h: csi related calculation (MS)
- _components/csi_component.h: gather, calc, print CSI

* Steps to setup
1. config SSID and password using ESP-IDF tools
2. config port and ip for ESP32 to send UDP periodically(to get CSI) to the router: find and change *ip and sin_port in [esp32-csi-tool/_components/sockets_component.h](esp32-csi-tool/_components/sockets_component.h)
3. config DEVICE_ID, port and ip for regular report: find function **http_post_data()** in [esp32-csi-tool/_components/http_client.h](esp32-csi-tool/_components/http_client.h). (Note: there are many unused functions in this file, only **http_post_data()** is used)
   * DEVICE_ID: the device id for the device, which is used to identify the device in the database and frontend
   * port and ip: the port and ip of the backend server is defined in a esp_http_client_config_t config.
4. burn the code to ESP32-S3
